...
/// <summary>
/// 排废工位请求处理器 — 验证容器 + 可选调用 MES/杭可 + 组装排废数据，不下发 WCS
/// 流程：验证容器 → 查 WasteBatchSetting → MES/杭可可选更新 → 组装排废字符串 → 返回 WcsResult
/// </summary>
public class WasteDisposalRequestHandler : IWcsRequestHandler
{
    ...
    private readonly ILogger<WasteDisposalRequestHandler> _logger;
    ...
    /// </summary>
    public string RequestType => Cst.排废;
    ...
    /// <param name="mesOptions"></param>
    /// <param name="hangkeOptions"></param>
    /// <param name="logger"></param>
    /// <exception cref="ArgumentNullException"></exception>
    public WasteDisposalRequestHandler(
        WmsDbContext db,
        ...
        ILogger<WasteDisposalRequestHandler> logger)
    {
    ...
    {
        // 1. 刷新 location（确保被 DbContext 跟踪）
        location = await _db.Locations.FindAsync(location.LocationId) ?? location;

        _logger.LogInformation("[WcsRequest] 排废验证: 位置={Location}, 容器={Container}",
        ...
        foreach (var containerCode in request.ContainerCode)
        {
            ...
            var setting = await _db.Set<WasteBatchSetting>()
            ...
            if (_mesOptions.Enable)
            {
                try
                {
                    ...
                    _logger.LogInformation("[WcsRequest] MES 排废信息更新完成: Container={Container}", containerCode);
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "[WcsRequest] MES 排废信息查询失败（不阻断流程）: Container={Container}", containerCode);
                }
            }
            ...
            if (_hangkeOptions.Enable)
            {
                try
                {
                    ...
                    _logger.LogInformation("[WcsRequest] 杭可排废信息更新完成: Container={Container}", containerCode);
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "[WcsRequest] 杭可排废信息查询失败（不阻断流程）: Container={Container}", containerCode);
                }
            }
            ...
            if (dicNo != null)
            {
                var parent = await _db.Set<BasicDictionary>()
                    .FirstOrDefaultAsync(x => x.No == dicNo);
                if (parent != null)
                {
                    var dictItems = await _db.Set<BasicDictionary>()
            ...
            _logger.LogInformation(
                "[WcsRequest] 排废验证完成: Container={Container}, WasteString={Waste}, NG={NgCount}/{Total}",
                containerCode, wasteString, ngCount, totalCount);
        }

        return ApiResultHelper.WcsSuccess("排废验证完成", ResultCodeTypes.一, 1, data: results);
    }

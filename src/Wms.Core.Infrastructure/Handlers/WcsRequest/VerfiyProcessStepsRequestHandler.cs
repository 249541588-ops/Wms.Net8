...
/// <summary>
/// 工艺验证工位请求处理器 — 比较多个托盘工艺次数是否一致，不下发 WCS
/// 流程：验证容器存在 → 取工艺次数 → 比较 → 返回 WcsResult
/// </summary>
public class VerfiyProcessStepsRequestHandler : IWcsRequestHandler
{
    private readonly WmsDbContext _db;
    private readonly ILogger<VerfiyProcessStepsRequestHandler> _logger;

    /// <summary>
    ///
    /// </summary>
    public string RequestType => Cst.工艺次数验证;
    ...
    public VerfiyProcessStepsRequestHandler(
        WmsDbContext db,
        ILogger<VerfiyProcessStepsRequestHandler> logger)
    {
    ...
    public async Task<WcsResult> HandleAsync(WcsRequestDto request, Location location)
    {
        ...
        _logger.LogInformation("[WcsRequest] 工艺次数验证: 位置={Location}, 容器={Container}",
            location.LocationCode, string.Join(",", request.ContainerCode ?? []));

        if (request.ContainerCode == null || request.ContainerCode.Length < 2)
            return ApiResultHelper.WcsFail("工艺次数验证至少需要两个容器编码", ResultCodeTypes.数据异常, -1);
        ...
        if (!allSame)
        {
            ...
            _logger.LogWarning("[WcsRequest] 工艺次数验证不通过: {Details}",
                string.Join(", ", details));
        ...
        _logger.LogInformation("[WcsRequest] 工艺次数验证通过，已更新位置: {Containers}→{Loc}",
            string.Join(",", codes), location.LocationCode);

        return ApiResultHelper.WcsSuccess("工艺次数验证通过", ResultCodeTypes.一, 1);
    }
}

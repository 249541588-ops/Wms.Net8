...
public interface ICtaskDbService
{
    ...
    /// <summary>
    /// 清理已完成处理的 WCS 任务（分批删除）
    /// </summary>
    Task<int> CleanupArchivedTasksAsync(int retentionDays);

    /// <summary>
    /// 根据 task_code 删除任务（用于回滚已下发的孤儿任务）
    /// </summary>
    /// <returns>实际删除的行数（0 表示无匹配）</returns>
    Task<int> DeleteTaskAsync(string taskCode);
}
